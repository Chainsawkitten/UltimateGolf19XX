...
void ParticleSystem::Render(const Camera* camera, const glm::vec2& screenSize) const {
    ...
    glBindTexture(GL_TEXTURE_2D, particleType.texture->TextureID());
    ...
    glUniformMatrix4fv(shaderProgram->UniformLocation("viewProjectionMatrix"), 1, GL_FALSE, &(camera->Projection(screenSize) * view)[0][0]);
    
    float alpha[3] = { particleType.startAlpha, particleType.midAlpha, particleType.endAlpha };
    glUniform1fv(shaderProgram->UniformLocation("alpha"), 3, alpha);
    
    glUniform3fv(shaderProgram->UniformLocation("color"), 1, &particleType.color[0]);
